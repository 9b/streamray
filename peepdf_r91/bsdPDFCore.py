...
class PDFBody :
    def __init__(self) :
        ...
        self.errors = []
        self.containsImage = False #BODY
        self.pageCount = 0
        self.containsEmbedded = False
        self.lang = None
        self.containsJs = False
        self.jsObjs = []
    ...
    #bsdixon - getter for containsImages - BODY
    def getContainsImages(self):
        return self.containsImage
        
    #bsdixon - getter for pageCount - BODY
    def getPageCount(self):
        return self.pageCount
        
    #bsdixon
    def getContainsEmbedded(self):
        return self.containsEmbedded 
        
    def getLang(self):
        return self.lang
        
    def getContainsJs(self):
        return self.containsJs
    ...
    def updateStats(self, id, pdfObject, delete = False):
        ...
        #bsdixon - track images count
        iMatch = re.compile("/Subtype /Image")
        if iMatch.search(value):
            self.containsImage = True
            
        #bsdixon
        pContents = []
        pMatch = re.compile("/Page\s") #simple find won't work    
        if value not in pContents: #track matches to avoid duplication
            if pMatch.search(value):
                self.pageCount += 1
            pContents.append(value)
            
        #bsdixon
        eMatch = re.compile("/EmbeddedFile")
        if eMatch.search(value):
            self.containsEmbedded = True
            
        #bsdixon - process an image stream if we find one
        iStreams = []
        t = re.compile("/Subtype /Image")
        if value not in iStreams:
            if t.search(value):
                image_hash = hashlib.sha1(pdfObject.getStream()).hexdigest()
                #TODO - implement the SSDeep portion and save these values in the object
            iStreams.append(value)
            
        try:
            lang = pdfObject.getElementByName("/Lang")
            self.lang = lang.getRawValue()[1:len(lang.getRawValue())-1] #removes ()
        except:
            pass
            
        #bsdixon
        def dictBreak(aObj,id,breaker=False):
            '''
                this function rips through every dictionary and follows it to the JS reference
            '''
            #print "breaker: " + str(breaker)
            
            try:
                if type(aObj.getElements()) == dict:
                    handle = aObj.getElements()
                    for k,v in handle.items():
                        if k == "/JavaScript" or k == "/JS":
                            #print "breaker set on " + k
                            breaker = True
                        else:
                            breaker = False
                            
                        dictBreak(handle[k],id,breaker)
            except Exception, e:
                 pass
                       
            try:
                if type(aObj.getElements()) == list:
                    for item in aObj.getElements():
                        dictBreak(item,id)                          	
            except Exception, e:
                #print e
                pass
            ...
            try:
                if breaker == True:
                    if hasattr(aObj, "getId"):
                        objId = aObj.getId()
                    #else:
                        #objId = id
                        
                    if objId not in self.jsObjs:
                        self.jsObjs.append(objId)
            except Exception, e:
                #print e
                pass
                
        #dictBreak(pdfObject,id)
        ...
        for action in monitorizedActions:
            if value.find(action) != -1:                    
                if action == "/JS" or action == "/JavaScript":
                    self.containsJs = True
...
class PDFFile :
    def __init__(self) :
        ...
        self.numDecodingErrors = 0
        self.maxObjectId = 0
        self.pageCount = 0 #bsdixon
        self.containsImages = [] #bsdixon - PDFfile
        self.lang = []
        self.containsJs = []
        self.containsEmbedded = []
        self.jsObjs = []
    ...
    #bsdixon
    def addContainsImages(self, containsImages):
        self.containsImages.append(containsImages)
        
    #bsdixon
    def addContainsEmbedded(self, containsEmbedded):
        self.containsEmbedded.append(containsEmbedded)
    
    #bsdixon
    def addLang(self, lang):
        if lang != None and lang not in self.lang:
            self.lang.append(lang)
        
    def addContainsJs(self, containsJs):
        self.containsJs.append(containsJs)
    
    def addJsObjs(self, jsObjs):
    	self.jsObjs = self.jsObjs + list(set(jsObjs) - set(self.jsObjs))
    ...
    def getBasicMetadata(self, version):
        ...
        if infoObject != None and infoObject.type != 'string':
            ...
            #bsdixon - added to account for additional metadata
            #bsdixon - TODO map these to XML stream components
            ...
            keywords = infoObject.getElementByName('/Keywords') 
            if keywords != None and keywords != []:
                  basicMetadata['keywords'] = keywords.getValue()			  	
            moddate = infoObject.getElementByName('/ModDate') 
            if moddate != None and moddate != []:
                    basicMetadata['modification'] = moddate.getValue() #modification to account for post-check
    ...
    def getStats (self):
        ...
        stats['Versions'] = []
        stats['Pages'] = str(self.pageCount) #bsdixon
        stats['Images'] = str(any(self.containsImages)) #bsdixon
        stats['Embedded Files'] = str(any(self.containsEmbedded))
        stats['Language'] = self.lang
        stats['JS'] = str(any(self.containsJs))
        stats['JS Objs'] = self.jsObjs
        for version in range(self.updates+1):
            statsVersion = {}
            catalogId = None
            infoId = None
            ...
            for id in ids:
                ...
                if object != None:
                    ...
                    if references == None:
                        ...
                        #bsdixon - can do type analysis here to identify the stream of JS
                        if type.find("/JavaScript") != -1 or type.find("/JS") != -1:
                            for id in referencesIds:
                                jObj = self.getObject(int(id), version)
                                if jObj != None:
                                    if jObj.getType() == 'stream':
                                        jStream = jObj.getStream()
                                
                        if type.find("/EmbeddedFile") != -1 or type.find("/Filespec") != -1:
                            for id in referencesIds:
                                eObj = self.getObject(id, version)
                                if eObj.getType() == 'stream':
                                    eStream = eObj.getStream()
    ...
    #bsdixon - define
    def setContainsImages(self, containsImages):
        self.containsImages = containsImages
        
    #bsdixon - define
    def addPageCount(self, pageCount):
        self.pageCount = pageCount
        
    #bsdixon - define
    def setContainsEmbedded(self, containsEmbedded):
        self.containsEmbedded = containsEmbedded
...
class PDFParser :
    ...
    def parse (self, fileName, forceMode = False, looseMode = False, streamed = False) :
        '''
        ...
        for i in range(len(self.fileParts)):
            ...
            pdfFile.addContainsImages(body.getContainsImages())
            pdfFile.addContainsEmbedded(body.getContainsEmbedded())
            pdfFile.addPageCount(body.getPageCount())
            pdfFile.addLang(body.getLang())
            pdfFile.addContainsJs(body.getContainsJs())
            pdfFile.addJsObjs(body.getJsObjs())
            isFirstBody = False
    ...
    def createPDFCrossRefSection (self, rawContent, offset):
        ...
        if pdfCrossRefSubSection != None:
            ...
            return (-1,'Error added subsection')
